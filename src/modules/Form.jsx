  ...
  return (
    <div className='bg-white w-[350px] rounded-lg lg:w-[600px] h-[550px] lg:h-[600px] flex flex-col justify-center items-center shadow-lg'>
      ...
      <form className='flex flex-col w-[100%] items-center justify-center' onSubmit={() => {console.log("Submitted")}}>
        {!isSignIn && <Input label={"Name"} name={"name"} placeholder={"Enter your name"} className={"mb-4"} value={data.name} onChange={(e) => setData({ ...data, name: e.target.value })} />}
        <Input label={"Email"} type='email' name={"email"} placeholder={"Enter your email"} className={"mb-4"} value={data.email} onChange={(e) => setData({ ...data, email: e.target.value })} />
        ...
        <Button label={isSignIn ? "Sign In" : "Sign Up"} type='submit' className={"w-1/3 lg:w-1/2 mb-4"} />
      </form>
      <div className='text-gray-400 text-sm'>{isSignIn ? "Didn't have an account? " : "Already have an account? "} <span className='text-primary cursor-pointer underline'> {isSignIn ? "sign up" : "sign in"}</span></div>
    </div>
  )
}
